 ...
 *
 * `0.1 + 0.2 !== 0.3` in IEEE-754, and a terminal that is a penny off on a
 * six-item order is a terminal that loses an audit. Cents in, cents out,
...
export const menuItems = pgTable(
  'menu_items',
  {
    ...
    /**
     * Provenance, carried per row so the report can state exactly where each
     * number came from -- and so a reviewer can check in one query which
     * prices are still estimates:
